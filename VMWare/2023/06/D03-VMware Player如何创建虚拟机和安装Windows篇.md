...
给虚拟机取一个名字，我用到现在应该没啥限制，中文英文都可以，带空格也支持。
...
我这里演示就配置了20G，实际情况根据自己的磁盘大小合理分配就可以。软件默认选择【将虚拟磁盘拆分成多个文件】这个选项，特点是不会直接占用实际磁盘的容量，比如我配置了20G，实际占用可能才几十M而已（未安装操作系统时），缺点就是性能损失。
...
打印机、声卡这些设备不用删除，作为桌面操作系统，这些都是可能用得到的。然后点击【关闭】按钮，继续。
...
`VMware Workstation Player 17`会打开新的一个页面，可能会有这么一个提醒【侧通道缓解】默认启用，VMware Player详细地解释了【侧通道缓解】作用，以及如何关闭。
...
首先虚拟机启动的时候会有一个VMware的Logo背景，很快就会出现一个提示，这个时候我们需要用鼠标点击下这个区域的任意一个地方，然后按下键盘的任意一个键就会引导光驱启动，否则就会出现下面的一个提示。
...
来到这个安装引导页面后，需要将硬盘分区，由于我们只分配了20G，不够分C盘、D盘，所以这里很简单，直接点击【下一步】按钮即可：
...
Windows 10 PC 端没啥用，我们可以点击【以后再说】按钮：
...
写这么详情的教程确实麻烦，麻烦大家点赞、在看和转发给有需要的朋友们吧。感谢！
